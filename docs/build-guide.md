...
`exports/ecmrs_table_columns.csv` and `exports/ecmrs_ido_properties.csv` hold the same schema as the table below, one row per field, with headers matching the real Application Studio fields (`Column Name`/`Data Type`/`Length`/`Nullable`/`Primary Key`/`Default Value` for the table; `Property Name`/`Bind To Column`/`Property Class`/`Key Sequence`/`Read Only`/`Required`/`Default From`/`List Source` for the IDO). Both are generated by `scripts/generate_schema_csv.py` from one master list, so they can never drift apart from each other or from `docs/field-mapping.md`.
