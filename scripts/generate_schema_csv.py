...
FIELDS = [
    ...
    ("initial_change", "InitialChange", "String", "40", "", "char", "", "", "", "Fixed 8-value list: Documentation/Machine/Material/Other/Process/Specification/Tooling/Variance(waiver) - add via the Inline List '...' editor, not this import. Drives the Requirements cascade."),
...
def main():
    ...
    with open("exports/ecmrs_ido_properties.csv", "w", newline="") as fh:
        ...
        for (col, pname, dtype, length, decimal, coldtype, labelid, required, readonly, desc) in FIELDS:
            # Column Data Type = same value as Data Type - matches what actually worked on the
            # Sql Columns side (plain base types), not the legacy QC-specific semantic names
            # this project originally guessed (QCSeq/QCInteger/QtyUnit all got rejected there).
            w.writerow([col, pname, "", dtype, length, decimal, dtype, labelid, required, readonly, desc])
